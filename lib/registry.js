 ...
 * @private
 *
 * Used by `getFullServiceName` to generate a unique identifier for
...
function _initManager() {
  ...
  self.manager.on('update', function (name, service) {
    ...
    if (self.services[name].update(service.data)) {
      self.emit('update', name, self.services[name]);
    } else {
      debug('Data unchanged, ignoring.');
    }
  });
  self.manager.on('available', function (name, service) {
    ...
    if (self.services[name]) {
      if (self.services[name].available) {
    ...
    self.emit('available', name, self.services[name]);
  });
  self.manager.on('unavailable', function (name, service) {
    ...
    self.emit('unavailable', name, self.services[name]);
  });
