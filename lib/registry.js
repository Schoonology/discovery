...
function _initManager() {
  ...
  self.manager.on('update', function (name, data, reason) {
    // TODO(schoon)
    //
    // self.services[name].update(data);
    // self.emit('update', name, self.services[name], reason);
  });
  self.manager.on('available', function (name, data, reason) {
    self.services[name] = new Service(data);

    self.emit('available', name, self.services[name], reason);
  });
  self.manager.on('unavailable', function (name, data, reason) {
